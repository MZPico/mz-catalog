...
export const ja: Ui = {
  ...
  privacy: {
    ...
    sections: [
      {
        h2: '要点',
        html: `Cookie、アクセス解析、広告は使っていません。他社のサーバーから読み込むものもありません。必要な機能を使ったときだけ、いくつかの情報をあなた自身のブラウザーに保存します。サーバーに残すのはソルト付きハッシュだけで、IP アドレスや識別子をそのまま保存することはありません。`,
      },
      {
        h2: '運営者',
      ...
      {
        h2: 'ブラウザーに保存するもの',
        html: `<ul><li><b>タッチ操作の配置</b>（<code>mz-touch-swap</code>）— スマートフォンで画面上の操作ボタンを入れ替えた場合のみ。</li><li><b>評価用 ID</b>（<code>mz-voter</code>）— 初めてタイトルを評価したときに作られる乱数。あとで評価を変更でき、一人が一票として数えられるようにするためのもので、あなたについての情報は含みません。</li></ul>これらはあなたの端末にだけ残り、ブラウザーでサイトのデータを消去すると削除されます。`,
      },
      {
        h2: 'サーバーに保存するもの',
        html: `<ul><li><b>評価</b>: タイトル、星の数（1〜5）、日時、評価用 ID とネットワークアドレスを合わせたソルト付きハッシュ、アドレスのみのソルト付きハッシュ（一つのネットワークから大量に投票できないようにするため）。評価が表示されている間保存します。</li><li><b>プレイ回数</b>: ネットワークアドレスのソルト付きハッシュとタイトル・日付。一つのネットワークは一日一回だけ数えます。その日が終われば削除し、タイトルごとの合計だけが残ります。</li><li><b>不正利用対策</b>: ハッシュ化したアドレスごと・一時間ごとの書き込み回数。一時間が過ぎれば削除します。</li></ul>ソルトは秘密なので、ハッシュからアドレスや ID を復元することはできません。法的根拠は、公正な評価を表示しサイトを不正利用から守るという正当な利益です（GDPR 第6条1項(f)）。`,
      },
      {
        h2: 'ホスティング',
        html: `サイトは Cloudflare 上で動いています。Cloudflare は処理者として、同社のデータ処理条件に基づき、ページの配信と攻撃からの保護のために IP アドレスとリクエスト情報を処理します。EU 域外での処理は、EU–米国データプライバシーフレームワークと標準契約条項に基づきます。Cloudflare の解析機能やログを訪問者の追跡には使っていません。`,
      },
      {
        h2: 'あなたの権利',
